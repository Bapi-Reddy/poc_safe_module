...
contract CreditAccountController is GearboxRegistry {
    ...
    uint256 public CURVE_ETH_STETH_SLIPPAGE = 50;
    uint256 public UNISWAP_ETH_FRAX_POOL_SLIPPAGE = 50;
    ...
    function _openCreditAccount(
        ...
        require(!isCreditAccountOpen(), "CA_ALREADY_EXISTS");
    ...
    function _swapETHToStETHCall(uint256 ethAmount, uint256 minStETHAmount)
        ...
        returns (MultiCall memory call)
    {
        uint256 stETHOutCurve = curvePool.get_dy(
            int128(uint128(CurvePoolIndex.ETH)),
            int128(uint128(CurvePoolIndex.STETH)),
            ethAmount
        );

        uint256 stETHOutLido = ethAmount - 1;

        if (stETHOutCurve > stETHOutLido) {
            //curve swap
            call = MultiCall({
    ...
    function positionInWantToken() public view returns (uint256 totalEquity) {
        if (isCreditAccountOpen()) {
            ...
            uint256 totalETH = borrowedAmountAndFeesAndInterest +
                address(creditAccount()).balance;
    ...
    /// @dev the ethPrice, stETHPrice are returned in USD with 8 decimal points.
    /// While stETHPriceOnCurve is returned with 4 decimal points.
    function getPrices()
        public
        view
        returns (
        ...
        returns (uint256 amountOut)
    {
        ...
        uint256 curveAmount = curvePool.get_dy(
            int128(uint128(CurvePoolIndex.STETH)),
            int128(uint128(CurvePoolIndex.ETH)),
            amountOfStETH
        );
    ...
    /// @dev This call will incase creditAccount is not yet opened by the time.
